...
class StudyBarChart extends StatelessWidget {
  ...
  // Build MA7 line spots từ pts
  List<FlSpot> get _ma7Spots => pts.asMap().entries.map((e) {
    return FlSpot(e.key.toDouble(), e.value.ma7.clamp(0.0, yMax));
  }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
              ...
              return BarChartGroupData(
                ...
                barRods: [
                  BarChartRodData(
                    ...
                    color: p.isGain
                        ? DashboardColors.gain.withOpacity(
                            isTouched ? 1.0 : 0.75,
                          )
                        : DashboardColors.loss.withOpacity(
                            isTouched ? 1.0 : 0.75,
                          ),
                  ),
                ],
        ...
        // ── MA7 Line Overlay ───────────────────────────────────────────────
        // Dùng IgnorePointer để touch vẫn xuống BarChart bên dưới
        IgnorePointer(
          child: LineChart(
            LineChartData(
              maxY: yMax,
              ...
              // Ẩn toàn bộ titles/grid/border — chỉ vẽ line
              titlesData: FlTitlesData(
                topTitles: const AxisTitles(
                  sideTitles: SideTitles(showTitles: false),
                ),
                ...
                leftTitles: AxisTitles(
                  sideTitles: SideTitles(
                    showTitles: true,
                    reservedSize: 32, // phải khớp BarChart để align
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    showTitles: true,
                    reservedSize: 18, // phải khớp BarChart để align
                    getTitlesWidget: (_, __) => const SizedBox.shrink(),
                  ),
                ),
              ),
              ...
              lineBarsData: [
                LineChartBarData(
                  ...
                  color: DashboardColors.primary, // màu tím như trong ảnh
                  barWidth: 2,
